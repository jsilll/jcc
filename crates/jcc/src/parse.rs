...
impl Ast {
    ...
    pub fn items_iter_both(&self) -> impl Iterator<Item = (ItemRef, &Item)> {
        self.items()
            .iter()
            .enumerate()
            .map(|(i, item)| (ItemRef(NonZero::new(i as u32 + 1).unwrap()), item))
    }
}
...
impl<'a> Parser<'a> {
    ...
    fn parse_body(&mut self) -> Option<BlockItemSlice> {
        let mut slice = BlockItemSlice::empty_at(self.result.ast.block_items.len());
        while let Some(Token { kind, .. }) = self.iter.peek() {
            match kind {
                TokenKind::RBrace => break,
                TokenKind::KwInt => {
                    let decl = self.parse_decl()?;
                    self.result.ast.push_block_item(BlockItem::Decl(decl));
                }
                _ => {
                    let stmt = self.parse_stmt()?;
                    self.result.ast.push_block_item(BlockItem::Stmt(stmt));
                }
            }
        }
        slice.update_end(self.result.ast.block_items.len());
        Some(slice)
    }
